...
async function logoutNetwork({ state, commit, network }) {
  ...
  const activeAuth = state.UAL[network].authenticators.find(
  ...
  if (activeAuth) {
    ...
    activeAuth
      ...
      .then(() => {
        ...
        commit("setSESSION", {
          data: { accountName: null, authenticatorName: null },
          network
        });
      })
      .catch(e => {
...
async function attemptAutoLoginNetwork({ state, commit, dispatch, network }) {
  if (
    ...
    typeof state.SESSION[network] === "undefined" ||
    state.SESSION[network] === null
  ) {
    return;
  }
  ...
  return new Promise((resolve, reject) => {
    if (accountName && authenticatorName) {
      ...
      console.log(
        `have account name and authenticator name ${accountName} ${authenticatorName}`
      );
      // dispatch('user/loggedInRoutine', accountName, { root: true })

      window.setTimeout(async () => {
        console.log("Timeout firing", state.UAL);
        const authenticator = state.UAL[network].authenticators.find(
          a => a.getStyle().text === authenticatorName
        );
        ...
        authenticator
          ...
          .then(async () => {
            ...
            await commit("setSESSION", {
              data: { accountName, authenticatorName },
              network
            });
...
export async function transact({ state, dispatch, commit }, payload) {
  ...
  console.log(
    `transact with stored state ${authenticatorName} ${accountName}@${permission}`
  );
...
function parseUalError(error) {
  ...
  if (error.cause) {
    cause =
      error.cause.reason ||
      error.cause.message ||
      "Report this error to the eosDAC devs to enhance the UX";
